...
class TagsOForm<T> implements FormModel<TagsO<T>, TagsOOutput<T>> {
  TagsOForm(this.form, this.path, this._formModel, {TagsO<T>? initialModel})
  ...
  TagsO<T>? _ownInitialModel;

  late Map<String, Object?> _ownInitialRawValue = TagsOForm.formElements(
    _ownInitialModel,
  ).rawValue;
  ...
  TagsO<T>? get initialModel {
    return _ownInitialModel;
  }

  void commitInitial([TagsO<T>? newModel]) {
    _ownInitialModel = newModel ?? rawModel;
    _ownInitialRawValue = TagsOForm.formElements(_ownInitialModel).rawValue;
  }
